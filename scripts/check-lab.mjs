...
try{
 for(const type of [chromium,webkit]){
  const browser=await type.launch();try{
   const context=await browser.newContext({viewport:{width:390,height:664},isMobile:true,hasTouch:true,locale:'ja-JP'});const page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));
   ...
   // Use a known sentinel to detect ANY dev-mode write to production storage.
   const normal=await page.evaluate(()=>localStorage.getItem('tamago2.care.v3'));
   await page.goto(base+'/?dev=1',{waitUntil:'networkidle'});await page.waitForFunction(()=>!!window.hitoikiLab);await page.locator('#lab-open').click();await page.locator('[data-lab="meal"]').click();await page.waitForTimeout(950);assert.equal(await page.locator('#lcd').getAttribute('data-reaction'),'eat');await page.screenshot({path:out+'/'+type.name()+'-lab-meal.png',fullPage:true});
   ...
   await page.locator('#lab-open').click();await page.locator('#lab-check-all').click();await page.waitForFunction(()=>document.querySelector('#lab-check-status').textContent.startsWith('完了'),{},{timeout:120000});const fails=await page.locator('#lab-check-results [data-result="fail"]').allTextContents();const passed=await page.locator('#lab-check-results [data-result="pass"]').count();assert.deepEqual(fails,[]);assert.ok(passed>60);await page.screenshot({path:out+'/'+type.name()+'-diagnostics.png',fullPage:true});
