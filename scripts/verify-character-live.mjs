...
for(let i=0;i<30;i++){
 try{
  const response=await fetch(base+'/version.json?t='+Date.now(),{signal:AbortSignal.timeout(12000)});const version=await response.json();if(version.features!=='character-polish-20260921-01')throw Error('waiting for release '+version.features);
